...
namespace InventoryManagementSystem
{
    public partial class UserModuleForm : Form
    {
       ...
       SqlConnection con=new SqlConnection(@"Data Source=DESKTOP-89C4245;Initial Catalog=testsql;Integrated Security=True;Connect Timeout=30");
        ...
        {
            try
            {//Passwordleri yoxlayriq ki,eger eyni deyilde warning qaytarsin
                ...
                if (MessageBox.Show("Are you sure you want to save this user?", "Saving Records", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes) 
                {
                    ...
                    cm.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("User has been saved succesfully");
                    Clear();
                }
            }
            catch(Exception ex)
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (MessageBox.Show("Are you sure you want to update this user?", "Update Records", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    cm.ExecuteNonQuery();
                    con.Close();
                    MessageBox.Show("User has been update succesfully");
                    this.Dispose();
                }
            }
            catch (Exception ex)
